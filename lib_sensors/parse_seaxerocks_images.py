...
class parse_seaxerocks_images:
	def __init__(self, filepath, sensor_string, date, camera1_label, camera2_label, camera3_label, category, timezone, timeoffset, ftype, outpath, fileoutname, fileout):
		...
		for i in range(len(camera3_filename)):# find corresponding timestamp even if some images are deletec
			...
			if ftype == 'oplab':					
				data = {'epoch_timestamp': float(epoch_timestamp_camera3[i]), 'class': class_string, 'sensor': sensor_string, 'frame': frame_string, 'category': category_laser, 'filename': str(camera3_filename[i])}
