	...
	def __init__(self, filepath, filename, category, timezone, timeoffset, headingoffset, ftype, outpath, fileoutname, fileout):
		...
		with codecs.open(filepath + filename,'r',encoding='utf-8', errors='ignore') as filein:
			...
			for line in filein.readlines():				
				...
				if line_split[2]!='Roll':
					if float(line_split[20])<altitude_limit:					
						if ftype == 'oplab':
							if category == 'velocity':

								frame_string = 'body'
													
								x_velocity=float(line_split[13]) # DVL convention is +ve aft to forward
								y_velocity=float(line_split[14]) # DVL convention is +ve port to starboard
								z_velocity=float(line_split[15]) # DVL convention is bottom to top +ve
								...
								east_velocity=float(line_split[9]) # phins convention is west +ve so a minus should be necessary
								north_velocity=float(line_split[10])									
								down_velocity=-9999
						...
						if ftype == 'acfr':
							if category == 'velocity':
								...
								xx_velocity=float(line_split[13]) # DVL convention is +ve aft to forward
								yy_velocity=float(line_split[14]) # DVL convention is +ve port to starboard
								zz_velocity=float(line_split[15]) # DVL convention is bottom to top +ve
