...
def projectPoint(sc, point3d):
    ...
    tvec = sc.t
    right_image_points = cv2.projectPoints(p, rvec, tvec, sc.right.P[0:3,0:3], sc.right.d)[0]
    ...
    if lx < w and rx < w and ly < h and ry < h and lx > 0 and rx > 0 and ly > 0 and ry > 0:
        return lx, ly, rx, ry
    else:
