...
def metres_to_latlon(latitude, longitude, eastings, northings):
    ...
    latitude_offset = math.asin(
        ...
        + math.cos(latitude) * math.sin(northings / R) * math.cos(0.0 * math.pi / 180.0)
    )
    longitude_offset = longitude + math.atan2(
        math.sin(90.0 * math.pi / 180.0) * math.sin(eastings / R) * math.cos(latitude),
        math.cos(eastings / R) - math.sin(latitude) * math.sin(latitude_offset),
    )
