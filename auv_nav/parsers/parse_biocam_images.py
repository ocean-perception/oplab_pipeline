...
def parse_biocam_images(mission,
    ...
    def timestamp_from_filename(filename, timezone_offset, timeoffset):
        filename_split = filename[i].strip().split('_')

        # 20190902_080209_205989_20190902_080207_636631_pcom.tif
    ...
    for i in range(len(camera1_filename)):
        t1, tc1 = timestamp_from_filename(camera1_filename[i],
                                          timezone_offset,
                                          timeoffset)
        stamp_pc1.append(str(t1))
        stamp_cam1.append(str(tc1))
    for i in range(len(camera2_filename)):
        t2, tc2 = timestamp_from_filename(camera2_filename[i],
                                          timezone_offset,
                                          timeoffset)
