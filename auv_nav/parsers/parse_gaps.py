            ...
            for line in gaps.readlines():
                        ...
                        if flag_got_time < 3:
                            ...
                            if line_split_no_checksum[6] == '0':
                                ...
                                latitude_minutes_ship_posterior = float(
                                    latitude_string[2:10])
                                latitude_posterior = latitude_degrees_ship_posterior + \
                                ...
                                longitude_minutes_ship_posterior = float(
                                    longitude_string[3:11])
                                longitude_posterior = longitude_degrees_ship_posterior + \
                ...
                if flag_got_time >= 5:
                    ...
                    longitude_ship = inter_time * \
                        (longitude_posterior-longitude_prior)+longitude_prior
                    latitude_ship = inter_time * \
                        (latitude_posterior-latitude_prior)+latitude_prior
                    heading_ship = inter_time * \
                    ...
                    lateral_distance_ship, bearing_ship = latlon_to_metres(
                        latitude_ship, longitude_ship, latitude_reference, longitude_reference)
                    eastings_ship = math.sin(
                        bearing_ship*math.pi/180.0)*lateral_distance_ship
                    northings_ship = math.cos(
                        bearing_ship*math.pi/180.0)*lateral_distance_ship

                    lateral_distance_target, bearing_target = latlon_to_metres(
                        latitude, longitude, latitude_reference, longitude_reference)
                    eastings_target = math.sin(
                        bearing_target*math.pi/180.0)*lateral_distance_target
                    northings_target = math.cos(
                        bearing_target*math.pi/180.0)*lateral_distance_target
                    ...
                    if broken_packet_flag == False:
                        ...
                        if ftype == 'acfr':
                            data = 'SSBL_FIX: ' + str(float(epoch_timestamp)) + ' ship_x: ' + str(float(northings_ship)) + ' ship_y: ' + str(float(eastings_ship)) + ' target_x: ' + str(float(northings_target)) + ' target_y: ' + str(
                                float(eastings_target)) + ' target_z: ' + str(float(depth)) + ' target_hr: ' + str(float(lateral_distance)) + ' target_sr: ' + str(float(distance)) + ' target_bearing: ' + str(float(bearing)) + '\n'
                            data_list += data
