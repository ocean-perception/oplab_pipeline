        ...
        if float(df['Height'][row_index]) < altitude_limit:
            if ftype == 'oplab':
                if category == 'velocity':
                    ...
                    data = {'epoch_timestamp': float(epoch_timestamp), 'epoch_timestamp_dvl': float(epoch_timestamp), 'class': class_string, 'sensor': sensor_string, 'frame': frame_string, 'category': category, 'data': [{'x_velocity': float(
                        x_velocity), 'x_velocity_std': float(x_velocity_std)}, {'y_velocity': float(y_velocity), 'y_velocity_std': float(y_velocity_std)}, {'z_velocity': float(z_velocity), 'z_velocity_std': float(z_velocity_std)}]}
                    ...
                    data = {'epoch_timestamp': float(epoch_timestamp), 'class': class_string, 'sensor': sensor_string, 'frame': frame_string, 'category': category, 'data': [{'north_velocity': float(north_velocity), 'north_velocity_std': float(
                        north_velocity_std)}, {'east_velocity': float(east_velocity), 'east_velocity_std': float(east_velocity_std)}, {'down_velocity': float(down_velocity), 'down_velocity_std': float(down_velocity_std)}]}
                    data_list.append(data)
                ...
                if category == 'orientation':
                    ...
                    if heading < 0:
                        ...
                        # write out in the required format interlace at end
                    data = {'epoch_timestamp': float(epoch_timestamp), 'class': class_string, 'sensor': sensor_string, 'frame': frame_string, 'category': category, 'data': [{'heading': float(
                        heading), 'heading_std': float(heading_std)}, {'roll': float(roll), 'roll_std': float(roll_std)}, {'pitch': float(pitch), 'pitch_std': float(pitch_std)}]}
                    data_list.append(data)
                    ...
                    data = {'epoch_timestamp': float(epoch_timestamp), 'class': class_string, 'sensor': sensor_string, 'frame': frame_string, 'category': sub_category, 'data': [{'heading_rate': float(
                        heading_rate), 'heading_rate_std': float(heading_rate_std)}, {'roll_rate': float(roll_rate), 'roll_rate_std': float(roll_rate_std)}, {'pitch_rate': float(pitch_rate), 'pitch_rate_std': float(pitch_rate_std)}]}
                    data_list.append(data)
                ...
                if category == 'depth':
                    ...
                    data = {'epoch_timestamp': float(epoch_timestamp), 'epoch_timestamp_depth': float(epoch_timestamp), 'class': class_string,
                            'sensor': sensor_string, 'frame': frame_string, 'category': category, 'data': [{'depth': float(depth), 'depth_std': float(depth_std)}]}
                    data_list.append(data)
                ...
                if category == 'altitude':
                    ...
                    # write out in the required format interlace at end
                    data = {'epoch_timestamp': float(epoch_timestamp), 'epoch_timestamp_dvl': float(epoch_timestamp), 'class': class_string, 'sensor': sensor_string, 'frame': frame_string, 'category': category, 'data': [
                        {'altitude': float(altitude), 'altitude_std': float(altitude_std)}, {'sound_velocity': float(sound_velocity), 'sound_velocity_correction': float(sound_velocity_correction)}]}
                    data_list.append(data)
            ...
            if ftype == 'acfr':
                if category == 'velocity':
                    ...
                    # print(data)
                    # write out in the required format interlace at end
                    data = 'RDI: ' + str(float(epoch_timestamp)) + ' alt:' + str(float(altitude)) + ' r1:0 r2:0 r3:0 r4:0 h:' + str(float(heading)) + ' p:' + str(float(pitch)) + ' r:' + str(float(roll)) + ' vx:' + str(
                        float(xx_velocity)) + ' vy:' + str(float(yy_velocity)) + ' vz:' + str(float(zz_velocity)) + ' nx:0 ny:0 nz:0 COG:0 SOG:0 bt_status:0 h_true:0 p_gimbal:0 sv: ' + str(float(sound_velocity)) + '\n'
                    data_list += data
                ...
                if category == 'orientation':
                    ...
                    if heading < 0:
                        ...
                        # print(data)
                        # write out in the required format interlace at end
                    data = 'PHINS_COMPASS: ' + str(float(epoch_timestamp)) + ' r: ' + str(float(roll)) + ' p: ' + str(float(pitch)) + ' h: ' + str(
                        float(heading)) + ' std_r: ' + str(float(roll_std)) + ' std_p: ' + str(float(pitch_std)) + ' std_h: ' + str(float(heading_std)) + '\n'
                    data_list += data
