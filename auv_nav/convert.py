...
def acfr_to_oplab(args):
    ...
    if args.vehicle_pose:
        ...
        if (csv_filepath / csv_filename).exists() and not force_overwite:
            ...
            Console.info(
                "Output vehicle pose written to", csv_filepath / csv_filename
            )
    if args.stereo_pose:
...
def oplab_to_acfr(args):
    ...
    camera0_nav_file = full_navigation_path / (
        "auv_ekf_" + camera0_name + ".csv"
    )
    ...
    camera1_nav_file = full_navigation_path / (
        "auv_ekf_" + camera1_name + ".csv"
    )
