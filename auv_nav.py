                    ...
                    if 'velocity' in parsed_json_data[i]['category']:
                        if 'body' in parsed_json_data[i]['frame']:
                            ...
                            if abs(parsed_json_data[i]['epoch_timestamp']-parsed_json_data[i]['epoch_timestamp_dvl']<1):
                                ...
                                time_velocity_body.append(parsed_json_data[i]['epoch_timestamp']) # dvl clock not necessarily synced by phins                           
        ...
        if plot is True:
            ...
            while i <= len(northings_usbl):
                ...
                if time_usbl[i] < time_velocity_body[0]:
                    ...
                    i = len(northings_usbl)
                                    

            for i in range(len(northings_dead_reckoning)):                
                northings_dead_reckoning[i]=northings_dead_reckoning[i]+northings_usbl[j]
                eastings_dead_reckoning[i]=eastings_dead_reckoning[i]+eastings_usbl[j]

            print(time_usbl[j],time_velocity_body[0])
            ...
            for i in range(len(northings_inertia_dead_reckoning)):  
                northings_inertia_dead_reckoning[i]=northings_inertia_dead_reckoning[i]+northings_usbl[j]
                eastings_inertia_dead_reckoning[i]=eastings_inertia_dead_reckoning[i]+eastings_usbl[j]

            print(time_usbl[j],time_velocity_inertia[0])
