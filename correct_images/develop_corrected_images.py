...
def develop_corrected_image(path_processed,path_mission,path_correct,force):
    '''
    ...
    path_mission = os.path.expanduser(path_mission)
    path_correct = os.path.expanduser(path_correct)
    path_processed = os.path.expanduser(path_processed)
    ...
    if os.path.isdir('./image') is True:
        if force is True:
            print('Processed images may already exist')
            print('Code will overwrite existing processed images')
            os.chdir(cwd)
        else:
            print('Code will quit - Processed images already exist')
            print('Run correct_images with [process] [-f] options for overwriting existing images')
            sys.exit()
    else:
        os.chdir(cwd)
        print('code will generate processed images for the first time')



    #params_dir_path = os.path.join(os.path.join(os.path.join(path_processed,"/"),img_path),"/attenuation_correction/params")
    params_dir_path = path_processed + '/' + img_path + '/attenuation_correction/params'
    #params_dir_path = os.path.expanduser(params_dir_path)
    
    # load filelist
    filelist_path = params_dir_path + '/filelist.csv'
    if os.path.exists(filelist_path) is True: 
    ...
    with open(path_config, 'r') as stream:
        # load_data_config = yaml.load(stream, Loader=yaml.FullLoader)
        load_data_config = yaml.load(stream)
    ...
    # load .npy files
    atn_crr_params = np.load(params_dir_path + '/atn_crr_params.npy')
    bayer_img_mean = np.load(params_dir_path + '/bayer_img_mean_raw.npy')
    bayer_img_std = np.load(params_dir_path + '/bayer_img_std_raw.npy')
    bayer_img_corrected_mean = np.load(params_dir_path + '/bayer_img_mean_atn_crr.npy')
    bayer_img_corrected_std = np.load(params_dir_path + '/bayer_img_std_atn_crr.npy')
    ...
    # get image size
    bayer_sample = np.load(list_bayer_file[0])
    ...
    if dst_dir_path is None:
        ...
        dst_dir_path = os.path.abspath(os.path.join(params_dir_path, os.pardir)) + '/developed'

    if os.path.exists(dst_dir_path):
        count = 2
        while True:
            dst_dir_path_cand = dst_dir_path + '_' + str(count).zfill(3)
            if not os.path.exists(dst_dir_path_cand):
                dst_dir_path = dst_dir_path_cand
                break
            else:
                count = count + 1
    ...
    for i_img in tqdm(src_img_index, ascii=True, desc=message):
        ...
        if apply_attenuation_correction:
            corrected_bayer_img = attenuation_correction_bayer(np.load(list_bayer_file[i_img]),
            ...
            corrected_bayer_img = pixel_stat_bayer(np.load(list_bayer_file[i_img]), bayer_img_mean, bayer_img_std,
        ...
        dst_path = dst_dir_path + '/' + os.path.basename(list_bayer_file[i_img]).replace('npy', dst_img_format)
        imageio.imwrite(dst_path, corrected_rgb_img)
        list_dst_name.append(os.path.basename(dst_path))
