...
class testCaseCorrector(unittest.TestCase):
    def test_calculate_parameters_RGB(self):
        ...
        self.assertEqual(
            (parameters_red_a_int == parameters_red_a_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along red channel",
        )
        self.assertEqual(
            (parameters_red_b_int == parameters_red_b_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along red channel",
        )
        self.assertEqual(
            (parameters_red_c_int == parameters_red_c_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along red channel",
        )
        self.assertEqual(
            (parameters_green_a_int == parameters_green_a_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along green channel",
        )
        self.assertEqual(
            (parameters_green_b_int == parameters_green_b_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along green channel",
        )
        self.assertEqual(
            (parameters_green_c_int == parameters_green_c_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along green channel",
        )
        self.assertEqual(
            (parameters_blue_a_int == parameters_blue_a_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along blue channell",
        )
        self.assertEqual(
            (parameters_blue_b_int == parameters_blue_b_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along blue channel",
        )
        self.assertEqual(
            (parameters_blue_c_int == parameters_blue_c_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels along blue channel",
        )

        self.assertEqual(
            np.array_equal(image_mean_blue_int, image_corrected_mean_blue_int),
            True,
            "Corrected mean not same for raw mean on blue channel",
        )
        self.assertEqual(
            np.array_equal(image_std_blue_int, image_corrected_std_blue_int),
            True,
            "Corrected mean not same for raw mean on blue channel",
        )
    ...
    def test_calculate_parameters_Black_White(self):
        ...
        self.assertEqual(
            (image_mean_int == 140).all(),
            True,
            "Mean values are incorrect and not same for all pixels",
        )
        self.assertEqual(
            (image_std_int == 74).all(),
            True,
            "Std values are incorrect and not same for all pixels",
        )

        self.assertEqual(
            (parameters_a_int == parameters_a_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels",
        )
        self.assertEqual(
            (parameters_b_int == parameters_b_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels",
        )
        self.assertEqual(
            (parameters_c_int == parameters_c_int[0, 0]).all(),
            True,
            "Parameter values are not same for all pixels",
        )

        self.assertEqual(
            (image_corrected_mean_int == image_corrected_mean_int[0, 0]).all(),
            True,
            "Corrected Mean values are not same for all pixels",
        )
        self.assertEqual(
            (image_corrected_std_int == image_corrected_std_int[0, 0]).all(),
            True,
            "Std values are not same for all pixels",
        )
