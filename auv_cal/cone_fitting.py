...
class CircularCone:
    ...
    def fit(self, points):
        ...
        ret = least_squares(self.residuals, coefficients, bounds=bounds, args=([points]), ftol=1e-3, xtol=1e-3, loss='huber', verbose=2, max_nfev=5000)
    ...
    def plot(self, length=10, cloud=None, points=None):
        ...
        X, Y, Z = [self.apex[i] + v[i] * t + R * np.sin(theta) * n1[i] + R * np.cos(theta) * n2[i] for i in [0, 1, 2]]
