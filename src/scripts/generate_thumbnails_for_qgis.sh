...
#
# Usage: ./generate_thumbnails_for_qgis.sh PATH_TO_FOLDER_WITH_IMAGES
#
#
...
if [[ -z "$d" ]];
    then echo "ERROR: No input provided. Usage: ./generate_thumbnails_for_qgis.sh PATH_TO_FOLDER_WITH_IMAGES";
    exit 1;
fi

if [[ "$d" == "-h" ]] || [[ "$d" == "--help" ]];
    then echo "Usage: ./generate_thumbnails_for_qgis.sh PATH_TO_FOLDER_WITH_IMAGES";
    exit 0;
fi
...
echo "$files" | parallel --eta mogrify -resize 200X169 -format jpg -path "${thumbnail_dir}" {} ||\
