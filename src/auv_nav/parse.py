...
from auv_nav.parsers.parse_sonardyne_mrt import parse_sonardyne_mrt, parse_sonardyne_gga, parse_sonardyne_gga2, parse_sonardyne_gga3, parse_sonardyne_gga4, parse_sonardyne_gga5, parse_sonardyne_gga6
...
def parse_single(filepath, force_overwrite):
    ...
    if not mission.usbl.empty():
        ...
        elif mission.usbl.format == "sonardyne_gga2":
            pool_list.append(
                pool.apply_async(
                    parse_sonardyne_gga2,
                    [mission, vehicle, "usbl", ftype, outpath],
                )
            )
        elif mission.usbl.format == "sonardyne_gga3":
            pool_list.append(
                pool.apply_async(
                    parse_sonardyne_gga3,
                    [mission, vehicle, "usbl", ftype, outpath],
                )
            )
        elif mission.usbl.format == "sonardyne_gga4":
            pool_list.append(
                pool.apply_async(
        ...
        elif mission.usbl.format == "sonardyne_gga6":
            pool_list.append(
                pool.apply_async(
                    parse_sonardyne_gga6,
                    [mission, vehicle, "usbl", ftype, outpath],
                )
            )        
        else:
